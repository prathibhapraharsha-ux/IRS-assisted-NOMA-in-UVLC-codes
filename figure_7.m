...
% Output arrays
P_out_ana_LOS      = zeros(length(zeta_val), length(gamma_dB));
P_out_ana_NLOS     = zeros(length(zeta_val), length(gamma_dB));
P_outage_M2        = zeros(length(zeta_val), length(gamma_dB));
P_outage_B_M2      = zeros(length(zeta_val), length(gamma_dB));
P_out_ana_auto     = zeros(length(zeta_val), length(gamma_dB));
P_out_B_M2         = zeros(length(zeta_val), length(gamma_dB));

P_out_mc_LOS       = zeros(length(zeta_val), length(gamma_dB));
P_out_mc_NLOS      = zeros(length(zeta_val), length(gamma_dB));
P_out_mc_M2        = zeros(length(zeta_val), length(gamma_dB));
P_out_mc_auto      = zeros(length(zeta_val), length(gamma_dB));
P_out_mc_B_M2      = zeros(length(zeta_val), length(gamma_dB));
P_outage_mc_B_M2   = zeros(length(zeta_val), length(gamma_dB));

N_sim = 1e5;

for j = 1:length(zeta_val)
    zeta = zeta_val(j);
...
semilogy(gamma_dB, P_out_ana_auto(3,:), 'b-',  'LineWidth', 2);
semilogy(gamma_dB, P_out_mc_auto(3,:),  'r^',  'LineWidth', 1.5);
semilogy(gamma_dB, P_out_B_M2(3,:), 'b--', 'LineWidth', 1.5);
semilogy(gamma_dB, P_outage_mc_B_M2(3,:), 'r^', 'LineWidth', 1.5);


xlabel('SNR (dB)');
ylabel('Outage Probability');
legend('Analytical for U_1 at \zeta = 1 ','','Analytical for U_2 at \zeta = 1 ','Simulation at \zeta = 1 ', ...
    'Analytical for U_1 at \zeta = 1.25 ','','Analytical for U_2 at \zeta = 1.25 ','Simulation at \zeta = 1.25', ...
    'Analytical for U_1 at \zeta = 1.5','','Analytical for U_2 at \zeta = 1.5 ','Simulation at \zeta = 1.5', ...
    'Location', 'southwest');
grid on;
xlim([20 50]);
