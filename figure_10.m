...
for i = 1:length(gamma_values)
    ...
    integrand_U_1_s = @(x) x.^(-1/2) .* exp(-x/4) .* P_out_ana_auto_s(i);
    aber_U_1_s(i) = (1/(4*sqrt(pi))) * integral(integrand_U_1_s,0,inf);

    integrand_U_1_f = @(x) x.^(-1/2) .* exp(-x/4) .* P_out_ana_auto_f(i);
    aber_U_1_f(i) = (1/(4*sqrt(pi))) * integral(integrand_U_1_f,0,inf);
    ...
    integrand_U_2_s = @(x) x.^(-1/2) .* exp(-x/4) .* P_out_B_M2_s(i);
    aber_U_2_s(i) = (1/(4*sqrt(pi))) * integral(integrand_U_2_s,0,inf);

    integrand_U_2_f = @(x) x.^(-1/2) .* exp(-x/4) .* P_out_B_M2_f(i);
    aber_U_2_f(i) = (1/(4*sqrt(pi))) * integral(integrand_U_2_f,0,inf);
    ...
    % Keep same analytical structure as your current outage code
    gamma_eff_LOS_s = max(gamma_X1_LOS_s, gamma_X2_LOS_s);

    % Monte Carlo ABER of LOS branch
    BER_inst_LOS_s = 0.5 * erfc(sqrt(gamma_eff_LOS_s / 4));
    aber_mc_LOS_s(i) = mean(BER_inst_LOS_s);

    gamma_X1_LOS_f = (eta * a_1 * rho * h_M11_f) ./ ((eta * epsilon * a_2 * rho * h_M11_f) + 1);
    gamma_X2_LOS_f = (eta * a_2 * rho * h_M11_f) ./ ((eta * a_1 * rho * h_M11_f) + 1);

    % Keep same analytical structure as your current outage code
    gamma_eff_LOS_f = max(gamma_X1_LOS_f, gamma_X2_LOS_f);

    % Monte Carlo ABER of LOS branch
    BER_inst_LOS_f = 0.5 * erfc(sqrt(gamma_eff_LOS_f / 4));
    aber_mc_LOS_f(i) = mean(BER_inst_LOS_f);
   ...
   % Keep same analytical structure as your current outage code
    gamma_eff_NLOS_s = max(gamma_X1_NLOS_s, gamma_X2_NLOS_s);

    % Monte Carlo ABER of NLOS branch
    BER_inst_NLOS_s = 0.5 * erfc(sqrt(gamma_eff_NLOS_s / 4));
    aber_mc_NLOS_s(i) = mean(BER_inst_NLOS_s);
    ...
    % Keep same analytical structure as your current outage code
    gamma_eff_NLOS_f = max(gamma_X1_NLOS_f, gamma_X2_NLOS_f);

    % Monte Carlo ABER of NLOS branch
    BER_inst_NLOS_f = 0.5 * erfc(sqrt(gamma_eff_NLOS_f / 4));
    aber_mc_NLOS_f(i) = mean(BER_inst_NLOS_f);

    aber_mc_U1_s(i) = aber_mc_NLOS_s(i) .* aber_mc_LOS_s(i);
    aber_mc_U1_f(i) = aber_mc_NLOS_f(i) .* aber_mc_LOS_f(i);
    ...
    gamma_eff_U2_s = max(gamma_X2_L2_s, gamma_B_X2_LOS_s);

    BER_inst_U2_s = 0.5 * erfc(sqrt(gamma_eff_U2_s / 4));
    aber_mc_U2_s(i) = mean(BER_inst_U2_s);
    ...
    gamma_eff_U2_f = max(gamma_X2_L2_f, gamma_B_X2_LOS_f);

    BER_inst_U2_f = 0.5 * erfc(sqrt(gamma_eff_U2_f / 4));
    aber_mc_U2_f(i) = mean(BER_inst_U2_f);
end

% ================= Plot =================
figure;

semilogy(gamma_dB, aber_U_1_s, 'b-',  'LineWidth', 2);hold on;
semilogy(gamma_dB, aber_mc_U1_s,  'bo',  'LineWidth', 1.5);
semilogy(gamma_dB, aber_U_1_f, 'r-',  'LineWidth', 2);hold on;
semilogy(gamma_dB, aber_mc_U1_f,  'ro',  'LineWidth', 1.5);

semilogy(gamma_dB, aber_U_2_s, 'b--', 'LineWidth', 1.5);
semilogy(gamma_dB, aber_U_2_s, 'bo', 'LineWidth', 1.5);
semilogy(gamma_dB, aber_U_2_f, 'r--', 'LineWidth', 1.5);
semilogy(gamma_dB, aber_U_2_f, 'ro', 'LineWidth', 1.5);
