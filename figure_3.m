...
for i = 1:length(gamma_values)
    ...
    % Final outage at U_1:
    P_out_ana_auto(i) = P_out_ana_LOS(i) *  P_out_ana_NLOS(i);
    ...
    P_out_mc_auto(i) =P_out_mc_NLOS(i) * P_out_mc_LOS(i);
...
semilogy(gamma_dB, P_out_ana_auto, 'k-',  'LineWidth', 2);hold on;
semilogy(gamma_dB, P_out_mc_auto,  'kp',  'LineWidth', 1.5);
semilogy(gamma_dB, P_out_ana_LOS, 'r-',  'LineWidth', 2);
semilogy(gamma_dB, P_out_mc_LOS,  'rp',  'LineWidth', 1.5);
semilogy(gamma_dB, P_out_B_M2, 'b-', 'LineWidth', 1.5);
semilogy(gamma_dB, P_outage_mc_B_M2, 'bp', 'LineWidth', 1.5);


xlabel('SNR (dB)');
ylabel('Outage Probability of U_1');
legend('Analytical for U_1 with IRS', 'Simulation for U_1 with IRS', ...
    'Analytical for U_1 without IRS', 'Simulation for U_1 without IRS', ...
    'Analytical for U_2', 'Simulation for U_2','Location', 'southwest');
grid on;
xlim([0 70]);
